# DiT transformer components adapted from FLOWER VLA
...
def find_multiple(n: int, k: int) -> int:
    """Returns the smallest number >= n that is a multiple of k."""
...
def modulate(x: torch.Tensor, shift: torch.Tensor, scale: torch.Tensor) -> torch.Tensor:
    """Applies modulation: x * (1 + scale.unsqueeze(1)) + shift.unsqueeze(1)"""
...
class SwiGlu(nn.Module):
    """SwiGlu MLP activation as used in transformer feedforward layers."""
...
def precompute_freqs_1d(dim: int, max_seq_len: int, theta: float = 10000.0) -> Tuple[torch.Tensor, torch.Tensor]:
    """Precompute cosine and sine frequency matrices for 1D rotary embeddings."""
...
class FlowerAttention(nn.Module):
    """Multi-head self-attention with optional rotary positional embeddings."""
    def __init__(self,
                 dim: int,
                 n_heads: int,
...
class FlowBlock(nn.Module):
    """Transformer block for flow-based diffusion with self-attn, cross-attn, and SwiGlu MLP with AdaLN."""
    def __init__(self,
                 dim: int,
                 heads: int = 8,
                 attn_pdrop: float = 0.1,
...
class ActionSpaceEmbedderParameter(nn.Module):
    ...
    def __init__(self, hidden_size, max_actions=11, embedding_size=256):
        super().__init__()
        self.action_embeddings = nn.Parameter(
            torch.randn(max_actions, embedding_size) * 0.02
        )
        self.mlp = nn.Sequential(
